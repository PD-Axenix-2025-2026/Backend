    ...
    *,
    ...
    if effective_query.max_price is not None:
        filtered_routes = [
            route
            for route in filtered_routes
            if _passes_max_price_filter(route, effective_query.max_price)
        ]
...
def build_price_range(routes: Sequence[RouteSnapshot]) -> DecimalRange:
    prices = list(_iter_known_total_price_amounts(routes))
    if not prices:
...
def _price_sort_key(route: RouteSnapshot) -> tuple[bool, Decimal, int, int, datetime]:
    return (
...
def _duration_sort_key(
    route: RouteSnapshot,
) -> tuple[int, bool, Decimal, int, datetime]:
    return (
        route.duration_minutes,
        not _has_known_total_price(route),
        _known_total_price_amount(route),
...
def _has_known_total_price(route: RouteSnapshot) -> bool:
    return route.total_price is not None


def _known_total_price_amount(route: RouteSnapshot) -> Decimal:
    if route.total_price is None:
        return Decimal("0")
    return route.total_price.amount


def _iter_known_total_price_amounts(
    routes: Sequence[RouteSnapshot],
) -> Iterator[Decimal]:
    for route in routes:
        total_price = route.total_price
        if total_price is not None:
            yield total_price.amount
